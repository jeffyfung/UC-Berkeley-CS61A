...
def balanced(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    left_torque, right_torque = None, None
    if is_planet(m):
        return True
    else:
        left_side = left(m)
...
def preorder(t):
    ...
    "*** YOUR CODE HERE ***"
    out_list = []
    def preorder_helper(sub):
        out_list.append(label(sub))
        if not (is_leaf(sub)):
            for b in branches(sub):
                preorder_helper(b)
    preorder_helper(t)
    return out_list
    ...
    def path_helper(t, out):
        if is_leaf(t):
            ...
            if out == word:
                ...
                return any([path_helper(b, out) for b in branches(t)])
    return path_helper(t, '')
    # seq = []
    # for i in preorder(t):
    #     seq += i
    #     return True if word == seq
...
def sub_interval(x, y):
    ...
    and any value in y."""
    "*** YOUR CODE HERE ***"
    p1 = add_interval(x, interval(-1 * lower_bound(y), -1 * upper_bound(y)))
    p2 = add_interval(x, interval(-1 * upper_bound(y), -1 * lower_bound(y)))
    return interval(min(lower_bound(p1), lower_bound(p2), upper_bound(p1), upper_bound(p2)),
                    max(lower_bound(p1), lower_bound(p2), upper_bound(p1), upper_bound(p2)))
