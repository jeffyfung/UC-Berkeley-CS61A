...
def berry_finder(t):
    ...
    if label(t) == 'berry':
        ...
        #print([b for b in branches(t)])
        return any([berry_finder(b) for b in branches(t)]) > 0
...
def sprout_leaves(t, leaves):
    ...
    #print('tree: ', t)
    if is_leaf(t):
        #print('yes its a leaf')
        leaves = [tree(l) for l in leaves]
        return tree(label(t), leaves)
    else:
        #print('branches: ', t)
        sub = [sprout_leaves(b, leaves) for b in branches(t)]
...
def riffle(deck):
    ...
    """
    "*** YOUR CODE HERE ***"
    half_point = len(deck) // 2
    return [deck[i//2] if i%2 == 0 else deck[half_point + i//2] for i in range(len(deck))]
