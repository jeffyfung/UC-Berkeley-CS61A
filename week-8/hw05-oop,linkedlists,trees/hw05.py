...
class Mint:
    ...
    def update(self):
        "*** YOUR CODE HERE ***"
        # set year stamp to current_year
        self.year = Mint.current_year
...
def is_bst(t):
    ...
    if t.is_leaf():
        ...
        return all(t.branches.map(is_bst))
    ...
    # cond1 = len(t_branches) <= 2
    # cond2 = t_branches[0].label <= t.label < t_branches[1].label
    # if cond1 and cond2:
    #     return is_bst(t_branches[0]) * is_bst(t_branches[1])
    # else:
    #     return False
...
def path_yielder(t, value):
    ...
    #         for b in t.branches:
    #             if b.label == value:
    #                 yield path + next(path_yielder)
    #                 yield path + next(path_yielder(b, value))
    #

    for b in t.branches:
        for br in b.branches:
            "*** YOUR CODE HERE ***"
            path_yielder(t, value)
            yield path
...
def order_list(link):
    ...
    while link.rest is not Link.empty:
        result = result * (link.first <= link.rest.first) # = min(link.first, link.rest.first), max(link.first, link.rest.first)
        link = link.rest
    return result
